...
const relayUrl = process.env.AWALBOT_RELAY || "http://localhost:3001";
...
if (!token) {
  ...
  Options (env vars):
    ...
    AWALBOT_RELAY   Relay server URL      (default: https://relay.awalbot.com)
`);
  process.exit(1);
}

connect({ token, relayUrl, openclawUrl, agentId });
